...
class DispatcherHandler(SocketServer.BaseRequestHandler):
    ...
    This will dispatch test runners against incoming commit
    ...
    def handle(self):
        ...
        if command == 'status':
            ...
            if not self.server.runners:
                ...
                # Thr coordinator can trust us to dispatch the test
                self.request.sendall('OK')
                dispatch_tests(self.server, commit_id)
        elif command == 'results':
            print 'got test results'
            results = command_groups.group(2)[1:]
            retults = results.split(':')
            commit_id = results[0]
...
def serve():
    ...
    print 'serving on %s:%s' % (args.host, int(atgs.port))
